...
public struct Mapper {
    ...
    public func from<T>(field: String) throws -> T {
        if let value = self.jsonFromField(field: field) as? T {
            return value
        }
    ...
    */
    ...
    public func from<T: RawRepresentable>(field: String) throws -> T {
        if let rawValue = self.jsonFromField(field: field) as? T.RawValue, let value = T(rawValue: rawValue) {
            return value
        }
    ...
    */
    ...
    public func from<T: Mappable>(field: String) throws -> T {
        if let json = self.jsonFromField(field: field) as? NSDictionary {
    ...
    */
     ...
     */
    public func from<T: Convertible>(field: String) throws -> [T] where T == T.ConvertedType {
        if let json = self.jsonFromField(field: field) as? [AnyObject] {
    ...
    */
    ...
    public func from<T>(field: String, transformation: (AnyObject?) throws -> T) rethrows -> T {
        return try transformation(self.jsonFromField(field: field) as AnyObject?)
    }
    ...
    public func optionalFrom<T>(field: String, transformation: (AnyObject?) throws -> T?) -> T? {
        return (try? transformation(self.jsonFromField(field: field) as AnyObject?)).flatMap { $0 }
    }
     ...
     */
    private func jsonFromField(field: String) -> Any? {
        return field.isEmpty ? self.json : self.json.value(forKeyPath: field)
    }
}
